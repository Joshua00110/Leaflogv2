...
// Action config for colors and icons
...
export default function EditPlantScreen() {
  ...
  const fetchPlant = async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        // Initialize schedules from data.schedules if present, or build from old watering fields
        const loadedSchedules: Record<string, ActionSchedule> = { ...(data.schedules || {}) };
        
        // If water schedule not present but old fields exist, create one
        if (!loadedSchedules.Water && data.wateringFrequency) {
          loadedSchedules.Water = {
            frequency: data.wateringFrequency,
            hour: data.reminderHour ?? 9,
            minute: data.reminderMinute ?? 0,
          };
        }
        
        // For other actions, if they are in tasks but have no schedule, create a default
        (data.tasks || []).forEach((action: string) => {
          if (action !== 'Water' && !loadedSchedules[action]) {
            loadedSchedules[action] = { frequency: 7, hour: 9, minute: 0 };
          }
        });
  ...
  const toggleAction = (action: string) => {
    setCareActions(prev => {
      ...
      // If adding action and it doesn't have a schedule yet, add default
      if (!prev.includes(action) && newActions.includes(action)) {
        setSchedules(prevSched => ({
          ...prevSched,
          [action]: { frequency: 7, hour: 9, minute: 0 }
        }));
      } else if (prev.includes(action) && !newActions.includes(action)) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // For backward compatibility, also include wateringFrequency and reminder fields
      if (schedules.Water) {
        updateData.wateringFrequency = schedules.Water.frequency;
        updateData.reminderHour = schedules.Water.hour;
        updateData.reminderMinute = schedules.Water.minute;
      }
        ...
        >
          ...
          >
            ...
            <Animated.View style={[styles.card, { transform: [{ scale: scaleAnim }] }]}>
              <View style={styles.cardHeader}>
                <View style={styles.cardIcon}>
                  <Ionicons name="information-circle-outline" size={24} color="#2E7D5E" />
                </View>
            ...
            <Animated.View style={[styles.card, { transform: [{ scale: scaleAnim }] }]}>
              <View style={styles.cardHeader}>
                <View style={styles.cardIcon}>
                  <Ionicons name="calendar-outline" size={24} color="#2E7D5E" />
                </View>
            ...
            <Animated.View style={[styles.card, { transform: [{ scale: scaleAnim }] }]}>
              <View style={styles.cardHeader}>
                <View style={styles.cardIcon}>
                  <Ionicons name="document-text-outline" size={24} color="#2E7D5E" />
                </View>
          ...
          {/* Schedule Modal with enhanced pickers */}
