    ...
    const fetchPlant = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          ...
          // For other actions, if they are in tasks but have no schedule, create a default (optional)
          (data.tasks || []).forEach((action: string) => {
            if (action !== 'Water' && !loadedSchedules[action]) {
              // Default schedule: every 7 days at 9:00
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        {/* Schedule Modal with separate hour and minute pickers */}
